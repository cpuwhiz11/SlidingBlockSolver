...
public class GameWindow {
	...
	private void setupPanel() {
		...
		// Add to panel
		lowerRow.add(new JLabel("DFS Options:"));
		...
		// Add to panel
		lowerRow.add(new JLabel("Heuristic options for A*")); 
		lowerRow.add(btnHeurOffset);
		lowerRow.add(btnHeurPlace); 
		
		// Blank spacer
		lowerRow.add(new JLabel("    "));
		...
		//imag.setIcon(new javax.swing.ImageIcon("E:\\cygwin\\home\\paul\\eclipseWorkspace\\SlidingBlockPuzzleSover\\src\\slidingBlockPuzzleSover\\res\\Imperial_aquila.jpg"));
		//this.mainPanel.add(imag); 
	}
	...
	private boolean moveNumber(int num) {
		...
		// If we can not do any of these moves the move is not near the empty space
		try {
	...
	public void printEndGameStats(ArrayList<Integer> moveList, double elapsedTime, int numInspected){
		...
		if (moveList.size() > 40){
			// Split up the lines on the label
			StringBuilder sb = new StringBuilder();
			
			sb.append("<html>The moves to win: ");
			
			int numOfSets = moveList.size() / 40; 
			int i = 0;
			int lastElemPos = 0;
			for (; i < numOfSets; i++){
				sb.append(moveList.subList(lastElemPos, i * 40 + 40).toString() + "<br>");
				lastElemPos = i * 40 + 41;
			}
			// Get the remaining numbers
			sb.append(moveList.subList(lastElemPos, moveList.size()).toString() + "</html>");
			
			aiMovesList.setText(sb.toString()); 
		} else {
			aiMovesList.setText("The move(s) to win: " + moveList.toString() + "\n");
		}
				
		numOfAiMoves.setText("That is : " + String.valueOf(moveList.size()) + " Move(s)\n");
	...
	/**
	 * Clean the stats labels  
	 */
	private void cleanLabels(){
		// Clean up labels if I needed
		aiMovesList.setText("");
		numOfAiMoves.setText("");
		aiTime.setText("");
		boardsIns.setText("");
	}
